...
项目在 6-8 月的迭代中实际长出了 PRD 非目标清单内的能力（认证体系、多套向量适配器、运维基础设施），定位从"展示 AI PM 能力的作品集 MVP"漂移向"追求工程完备的准生产系统"。2026-08-12 完成两项桌面调研（代码库理解/onboarding 赛道、影响分析与 LLM 评估赛道）后，做出如下收敛决策。

### 决策一：品类锚定不变

保持「AI Developer Onboarding Copilot / 代码库理解助手」的主流品类定位。调研显示「仓库导入 → 自动 Wiki → 带引用问答」范式已被巨头验证并趋于标配：Google Code Wiki（2025-11 公开预览）、Cognition DeepWiki（已索引 5 万+ 仓库）、GitHub Copilot Spaces（2025-09 GA）。**品类拥挤对本项目是利好而非威胁**——作品集不需要护城河，需要的是"选题判断被市场同向验证"；RAG 问答模块定位为及格线组件，不作为卖点。

### 决策二：两个差异化点定位为"亮点"，不是"身份"

1. **面向 PM/QA 的改动影响简报**：调研确认技术侧影响分析工具（CodeScene/Moderne/Launchable/SeaLights）全部面向工程师与 CI 流水线，AI 代码审查产品（Greptile/CodeRabbit 等）无一将非技术角色列为目标用户；「需求阶段自然语言输入 → 受影响模块/业务链路/风险级别/测试重点的非工程师可读输出」无直接对标产品。此点作为**用户洞察**的证据呈现。
2. **产品内置 AI 质量看板**：LLM 评估平台（LangSmith/Langfuse/Braintrust/Arize）均为外挂第三方工具且赛道拥挤；但"PM/QA 无需工程介入即可查看评估"正成为行业共识方向（Confident AI 2026 评测报告）。本项目把引用率/schema 合规率/guardrail 命中等指标内嵌为产品自身的信任设计，作为**指标定义与产品化能力**的证据呈现，并在叙事中明确"为什么自建而非接入现成 LLMOps 工具"。

### 决策三：停止投入清单

以下能力**保留现状、停止加码**（价值定格为"懂工程边界"的素养证据）：认证与授权体系、外部向量库适配器（Qdrant/Pinecone/HTTP）、运维与部署基础设施。对照第 3 节 MVP 非目标执行。

### 决策四：后续投入方向（按优先级）

1. 影响分析输出从技术清单向「业务链路自然语言简报」打磨（服务 PM/QA persona）；
2. 评估看板从指标陈列升级为「数据故事」（展示至少一次指标驱动的迭代闭环）；
3. 面试配套材料（positioning one-pager 已建：docs/POSITIONING.md）。

### 风险跟踪

- Google Code Wiki 私有仓库/企业版仍在等待名单阶段，开放后将重塑赛道格局（建议每季度复查）；
- AI 代码审查产品随手新增"给 PM 的摘要"无技术门槛——本项目护城河在产品判断与组合叙事，面试叙事围绕此点展开；
- 「PM/QA 影响简报」的空白可能源于需求刚性不足而非无人想到，面试中主动、诚实地讨论此不确定性。
