...
// ── 3. Routing: human_review → supervise → synthesize ──
assert(serverSource.includes('nextNode === "qa_plan" && state.riskLevel === "high" && AGENT_HITL_ENABLED'), "HITL routing gate must exist (high risk + enabled → human_review)");
...
console.log("[OK] Routing: high risk + HITL enabled → human_review → synthesize.");
