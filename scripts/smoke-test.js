...
async function main() {
  ...
  try {
    ...
    const checkpointForAudit = agentRunAudit.checkpoints.find((item) => item.checkpoint_id && item.state_summary);
    assert(checkpointForAudit, "harness run audit did not include checkpoint id for time-travel inspection");
    const langgraphCheckpoint = await request(`/api/langgraph-checkpoint?projectId=${encodeURIComponent(projectId)}&runId=${encodeURIComponent(agent.payload.harness.run_id)}&checkpointId=${encodeURIComponent(checkpointForAudit.checkpoint_id)}`);
    assert(langgraphCheckpoint.checkpoint.checkpoint_id === checkpointForAudit.checkpoint_id, "single checkpoint audit returned wrong checkpoint id");
    assert(langgraphCheckpoint.checkpoint.run_id === agent.payload.harness.run_id, "single checkpoint audit returned wrong run id");
    ...
    const resumedAgent = await request("/api/langgraph-resume", {
      method: "POST",
      body: JSON.stringify({
        projectId,
        runId: agent.payload.harness.run_id,
        checkpointId: checkpointForAudit.checkpoint_id
      })
    });
    assert(resumedAgent.answerId !== agent.answerId, "LangGraph resume should create a new answer");
    ...
    assert(missingResumeRun.status === 400, "missing resume run id should return 400");
    assert(missingResumeRun.payload.code === "RUN_ID_REQUIRED", "missing resume run id returned wrong code");
    ...
    const missingCheckpoint = await requestError(`/api/langgraph-checkpoint?projectId=${encodeURIComponent(projectId)}&runId=${encodeURIComponent(agent.payload.harness.run_id)}&checkpointId=missing-checkpoint`);
    assert(missingCheckpoint.status === 404, "missing checkpoint should return 404");
    assert(missingCheckpoint.payload.code === "LANGGRAPH_CHECKPOINT_NOT_FOUND", "missing checkpoint returned wrong error code");
