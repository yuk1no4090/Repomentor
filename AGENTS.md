...
- 后端已完成模块化拆分：`server.js` 从单文件 6029 行降到约 1213 行，只保留 HTTP 路由分发（`handleApi`/`handleApiUnlocked`/`serveStatic`）、少量仍与路由强耦合的辅助函数（`sendJson`/`readBody`/`findHarnessRunAudit`）和 bootstrap（`setStoreRecordNormalizers`/`setCheckpointCollaborators` 接线、`http.createServer`、优雅关机）。全部业务逻辑迁到 `lib/` 下 13 个单一职责模块：`lib/config.js`（环境变量+配置常量+共享工具函数）、`lib/store.js`（store 读写+常驻内存缓存+写锁）、`lib/memory-db.js`（SQLite 长期记忆全部：迁移/FTS/embedding/向量适配器/备份恢复）、`lib/checkpoints.js`（LangGraph checkpoint 序列化/持久化/回放/续跑）、`lib/auth.js`（token/身份解析、认证用户与事件、scope 校验）、`lib/importer.js`（仓库导入：ZIP/GitHub/项目创建/技术栈推断/导入安全扫描）、`lib/retrieval.js`（分词/查询扩展/分块/检索）、`lib/safety.js`（输入/检索/输出安全扫描与脱敏）、`lib/llm.js`（模型 provider 解析+普通/角色化模型适配器）、`lib/agent-contracts.js`（Supervisor/ImpactAnalyst/QACritic 独立 prompt、schema 与回退）、`lib/answers.js`（QA/Impact/Onboarding 确定性答案生成+偏好增删查+记忆建议生成）、`lib/agent-graph.js`（路由决策+trace/工具注册表+harness 报告构造器+StateGraph 节点+`runAgenticImpactWorkflow`）、`lib/metrics.js`（`computeMetrics`+`FEEDBACK_TYPES`）。`lib/` 模块间依赖单向无环，跨层协作（`server.js` 的记录归一化函数注入 `lib/store.js`、`findProject`/`findHarnessRunAudit`/`runAgenticImpactWorkflow` 注入 `lib/checkpoints.js`）通过 `setStoreRecordNormalizers()`/`setCheckpointCollaborators()` 两个注入点完成，避免循环依赖。详见 [docs/AGENT_RUNTIME_ARCHITECTURE.md](docs/AGENT_RUNTIME_ARCHITECTURE.md) 的「Code Organization」小节。
...
`npm test` 依次跑 10 个子命令，全程不需要真实的 LLM API key —— 没有配置 `OPENAI_API_KEY` 时，agent 工作流和 harness 会走确定性离线检索/回退路径；smoke test 还会自带一个 fake OpenAI-compatible LLM server 用来验证有 key 时的行为（schema 校验失败、超时、越权引用等边界）。
...
- `test/` 目录下是 Node 内置 `node:test` 运行的 100 条单元测试，覆盖路由、安全、检索、Agent contracts、checkpoint 保留、偏好纯读、briefing、前端导入 helper 和工作流超时，直接导入真实函数调用断言，不是重新实现一份逻辑镜像去测那份镜像。
