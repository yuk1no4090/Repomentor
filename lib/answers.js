...
// Pure read: does not create store.userPreferencesByUser[userId] as a side
// effect. It used to lazily write a normalized-defaults entry into the store
// on first access for a given userId, but every current call site (server.js
// route handlers, createMemorySuggestions() below, the LangGraph "memory"
// node and runAgenticImpactWorkflow() in lib/agent-graph.js) only reads the
// returned value — nothing else reads store.userPreferencesByUser[userId]
// directly, so nothing depends on that write. Several of those call sites
// now run during the unlocked compute phase of server.js's heavy-mutation
// routes (see isHeavyMutationRoute()), where a hidden store write here would
// race with concurrent requests outside any lock. setUserPreferences() below
// remains the only place that creates/updates a store-backed preferences
// entry, and it is only ever called from routes that already hold the write
// lock for their full duration.
function getUserPreferences(store, userId = DEFAULT_USER_ID) {
  ...
  const stored = store.userPreferencesByUser?.[normalizedUserId]
    ?? (normalizedUserId === DEFAULT_USER_ID ? store.userPreferences : null);
