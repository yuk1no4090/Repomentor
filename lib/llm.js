...
{"summary": "<non-empty string>",
 ...
 "briefing": {"summary": "<2-4 sentence plain-language business narrative for a non-technical PM/QA reader - no function names, no line numbers>", "affected_flows": [{"flow": "<business process name, e.g. \\"Login & permissions\\">", "why": "<plain-language reason this flow is affected>"}], "testing_focus": ["<plain-language behavior to verify, not a technical test-implementation detail>"], "risk_note": "<plain-language risk explanation plus a recommended decision>"}}
Every impact_areas entry must be an object carrying all four keys; risk_level must be exactly one of low, medium, or high. Include "briefing" so a reader with no coding background can understand the change: briefing.affected_flows must be an array of objects (never bare strings), each with both flow and why as non-empty strings; briefing.testing_focus must be an array of strings; briefing.risk_note must be a non-empty string.`
    : `Return JSON matching exactly this shape:
