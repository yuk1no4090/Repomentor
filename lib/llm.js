...
//
// Only plain decimal literals are accepted -- checked with this regex BEFORE
// ever calling Number(). Number("0x2") is 2, Number("0b1") is 1,
// Number("0o10") is 8: the bare `Number(trimmed)` this used to call happily
// parses hex/binary/octal literals, so an operator's typo like
// `OPENAI_TEMPERATURE_QA_CRITIC=0x2` silently became temperature 2 instead of
// being rejected as malformed. Rejecting anything the regex doesn't match up
// front -- falling through to the same "treat as absent" path as today's
// NaN/out-of-range handling -- closes that off without changing behavior for
// any value operators actually write in practice.
//
// The pattern is `/^[+-]?(\d+\.?\d*|\.\d+)$/`, not the narrower
// `/^-?\d+(\.\d+)?$/` this originally shipped with: that narrower form
// rejected forms a human operator plausibly types by hand -- a leading-dot
// decimal (".5"), an explicit leading `+` ("+1"), and a trailing-dot integer
// ("1.") -- as malformed, which was an undocumented behavior change from the
// pre-regex `Number()` call (which accepted all three). The two alternatives
// split on where the digits sit relative to the (optional) `.`:
// `\d+\.?\d*` covers "1", "1.", and "1.5"; `\.\d+` covers ".5" (at least one
// digit is required somewhere in every case, so a bare "+", "-", or "." alone
// still cannot match). Both alternatives still require ONLY digits, an
// optional single leading sign, and at most one `.` -- so 0x/0b/0o prefixes
// and scientific notation remain rejected exactly as before.
//
...
const TEMPERATURE_DECIMAL_PATTERN = /^[+-]?(\d+\.?\d*|\.\d+)$/;
