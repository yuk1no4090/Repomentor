...
import { DEFAULT_USER_ID, CHECKPOINT_MAX_RUNS, apiError, normalizeUserId } from "./config.js";
...
async function persistLangGraphCheckpoints({ projectId, runId, threadId, checkpointer, resumeInput = null }) {
  ...
  pruneOldLangGraphCheckpointRuns(db);
  return {
    enabled: true,
    saver: "MemorySaver",
    persisted: true,
...
// Keeps memory.sqlite bounded: every persistLangGraphCheckpoints() call writes
// a full base64 MemorySaver snapshot (langgraph_checkpoint_payloads, one row
// per run) plus a row per checkpoint step (langgraph_checkpoints), and neither
// table was ever pruned — a long-running deployment accumulates one full
// snapshot per agent-impact run forever. After each successful persist, drop
// every run outside the most recent CHECKPOINT_MAX_RUNS runs (ranked by that
// run's most recent activity across both tables, so a run is never pruned from
// one table while surviving in the other).
//
// The "most recent activity" ranking combines langgraph_checkpoints' per-step
// `created_at` (the checkpoint's own timestamp) with
// langgraph_checkpoint_payloads' `created_at` (set once, when that run's
// payload is written) rather than relying on either table alone: a run whose
// graph produced zero checkpoint steps (defensive edge case — normal graph
// execution always saves at least one) would otherwise vanish from a
// checkpoints-only ranking the moment its payload is persisted, deleting the
// payload we just wrote in the very same call.
function pruneOldLangGraphCheckpointRuns(db) {
  const rankedRunIds = `
    SELECT run_id FROM (
      SELECT run_id, MAX(created_at) AS latest FROM langgraph_checkpoints GROUP BY run_id
      UNION ALL
      SELECT run_id, created_at AS latest FROM langgraph_checkpoint_payloads
    ) combined_runs
    GROUP BY run_id
    ORDER BY MAX(latest) DESC
    LIMIT ?
  `;
  const pruneCheckpoints = getCachedStatement(db, `
    DELETE FROM langgraph_checkpoints WHERE run_id NOT IN (${rankedRunIds})
  `);
  const prunePayloads = getCachedStatement(db, `
    DELETE FROM langgraph_checkpoint_payloads WHERE run_id NOT IN (${rankedRunIds})
  `);
  runInSqliteTransaction(db, () => {
    pruneCheckpoints.run(CHECKPOINT_MAX_RUNS);
    prunePayloads.run(CHECKPOINT_MAX_RUNS);
  });
}
...
async function runLangGraphResumeFromCheckpoint(store, { projectId, runId, checkpointId = null, userId = DEFAULT_USER_ID, decision = null } = {}) {
  ...
  // Three resume modes (see runAgenticImpactWorkflow() in lib/agent-graph.js for the
  // matching graphInput/checkpointer selection for each):
  //  - native_interrupt_resume: a persisted MemorySaver payload survives AND a decision
  //    was supplied — rehydrate it and resume the SAME paused execution via
  //    Command({ resume: decision }); only human_review + synthesize re-run.
  //  - checkpoint_continuation: a persisted payload survives but no decision was
  //    supplied — replay the paused checkpoint verbatim (interrupt() re-fires
  //    immediately; the run stays paused).
  //  - input_snapshot_reexecution: legacy fallback for when the payload was pruned
  //    (CHECKPOINT_MAX_RUNS) — re-executes the saved input snapshot from phase 0 with
  //    the decision injected directly into the initial state.
  const resumeMode = checkpointPayload
    ? (decision ? "native_interrupt_resume" : "checkpoint_continuation")
