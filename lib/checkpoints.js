...
import { getMemoryDatabase } from "./memory-db.js";

// ── Collaborators owned by server.js ──
// buildLangGraphReplay()/findLangGraphCheckpoint()/runLangGraphResumeFromCheckpoint()
// need findProject(), findHarnessRunAudit() and runAgenticImpactWorkflow() — all
// core project/agent-workflow business logic that stays in server.js for this
// storage-layer split (see refactor report). Since lib modules may not import
// from server.js (that would create a server.js -> lib/checkpoints.js ->
// server.js cycle), server.js injects these once at startup via
