...
// ── HITL trigger signals (Task N2) ──────────────────────────
// Two independent, OR'd signals can request a pause before "synthesize": the
// ImpactAnalyst's evidence-grounded riskLevel (state.riskLevel === "high" --
// the original, and still primary, trigger) and the Supervisor's own
...
// lib/agent-contracts.js, but until this card, schema-enforced and then
// consumed by nothing: a signal the Supervisor's own planning role could
// contribute to safety, silently discarded). risk_hypothesis (the
...
// look at this" verdict, so it is the one signal this card adds.
//
...
// -- one source of truth for "did either signal fire", not two independently
...
// costs that conformant path nothing.
...
// is the primary/original trigger) so a pause caused by both signals reads
// as one combined sentence instead of just the last-computed one.
function describeHitlReason(triggers) {
  return triggers.map((trigger) => HITL_TRIGGER_REASON_TEXT[trigger]).join("; ");
}
  ...
  if (AGENT_GRAPH_MODE === "linear") {
    ...
    graph = graph
      ...
      .addNode("human_review", async (state) => {
        ...
        // Task N2: reviewRequest.reason/triggers name WHICH signal(s) fired
        // (riskLevel="high" and/or the Supervisor's own require_human_review
        // flag -- see hitlReviewTriggers()/describeHitlReason() above), so an
        // operator looking at the interrupt() payload (or the mirrored
        // hitl_paused SSE event, or the paused answer's hitl.reason/
        // hitl.triggers) can tell why this run stopped instead of seeing only
        // a generic "high risk" message even when it was the Supervisor's
        // own plan that asked for review.
        const triggers = hitlReviewTriggers(state);
        const reviewRequest = {
          reason: describeHitlReason(triggers),
          triggers,
...
async function runAgenticImpactWorkflow(store, project, question, userId = DEFAULT_USER_ID, resumeMetadata = null, options = {}) {
  ...
  try {
    ...
    if (isInterrupted(state)) {
      // Task N2: recomputed via the SAME pure hitlReviewTriggers()/
      // describeHitlReason() helpers human_review's own reviewRequest used --
      // `state` here is the identical pre-pause state (riskLevel,
      // supervisorPlan) human_review saw, so this is guaranteed to agree with
      // what interrupt() was actually called with, not an independently
      // maintained second computation that could drift from it.
      const triggers = hitlReviewTriggers(state);
      const pausedTrace = [
        ...state.trace,
        makeTraceStep({
      ...
      const pausedState = {
        ...state,
        hitlRequest: { node: "human_review", reason: describeHitlReason(triggers), checkpoint_id: null, decision: null, triggers },
        trace: pausedTrace
      };
