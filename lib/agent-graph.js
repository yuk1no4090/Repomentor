...
// ── HITL trigger signals (Task N2, extended by Task N3, extended by Task N4) ─
...
// contribute to safety, silently discarded), (Task N3) the deterministic
// safety layer's own detectors in lib/safety.js, and (Task N4) the QACritic's
// own unresolved "revise" verdict. risk_hypothesis (the Supervisor's own risk
// guess, also on supervisorPlan) is deliberately NOT wired in here -- it stays
// advisory/observability-only. require_human_review is the one field the
// schema already casts as an explicit "should a human look at this" verdict,
// so it is the one signal Task N2 added.
//
...
//
// Task N4 adds a fourth: the QACritic's own verdict, but ONLY the exact
// moment the bounded revise loop (see the revise-branch guard in
// decideNextRoute below) could not resolve it. `qa_plan`'s verdict="revise"
// is not, by itself, a safety problem -- it is the loop's own steady-state
// signal that drives one more retrieve/expand_context/impact_analysis/qa_plan
// round, and if that round's critic then approves, the loop simply continues
// to "guardrails" with no pause at all (see the "cursor correctness after a
// loop" routing test). The moment that DOES warrant a human is when the
// pipeline reaches the guardrails->synthesize gate with the critic STILL
// saying "revise" and no budget left to try again -- i.e. it is about to ship
// an answer the critic itself still rejects. That is exactly
// `state.qaReview?.verdict === "revise" && (state.revisionRound || 0) >=
// AGENT_MAX_REVISION_ROUNDS` -- the logical negation of the revise-branch
// guard's own `< AGENT_MAX_REVISION_ROUNDS` condition (mirrored deliberately;
// see buildAgentHarnessReport's pre-existing `revision_budget_exhausted`
// field above, which already computes this identical predicate for
// observability -- critic_flag is that same condition now made
// ROUTING-relevant, not a second independently-invented threshold). This is
// NOT "findings severity == high" or "any revise verdict ever seen" -- the
// deterministic critic (createDeterministicQaCriticReview in
// lib/agent-contracts.js) emits a high-severity finding for ANY uncited
// impact area, which is common on ordinary offline runs; triggering on that
// would fire on nearly every degenerate/offline fixture and defeat the whole
// point of a *bounded* revise loop by turning routine convergence noise into
// a pause. Triggering on "unresolved revise verdict at the terminal gate"
// instead targets only the moment of genuine, un-worked-out disagreement
// between the critic and the rest of the pipeline.
//
...
//
// Canonical trigger order (pinned by the routing unit tests and by every
// behavioral fixture that asserts an exact triggers array): the four `if`
// statements below run in DECLARATION order and each simply pushes onto the
// same array, so the order below IS the canonical order --
// ["high_risk", "supervisor_flag", "input_safety_flag", "retrieved_safety_flag",
// "critic_flag"] -- new signals are appended at the end, in the order the
// task that introduced them was done (N1/N2 -> N2 -> N3 -> N3 -> N4), rather
// than interleaved by phase/pipeline position (which would make the order a
// second, independently-moving thing every future addition would have to
// reconcile against pipeline ordering instead of just appending).
function hitlReviewTriggers(state) {
  ...
  if (state.qaReview?.verdict === "revise" && (state.revisionRound || 0) >= AGENT_MAX_REVISION_ROUNDS) triggers.push("critic_flag");
  return triggers;
}
...
//
// Task N4: critic_flag similarly needs the round count that made the revise
// loop unresolved (a bare "critic requested revision" loses the "budget
// exhausted, not just mid-loop" distinction that is the whole point of this
// trigger -- see hitlReviewTriggers()'s own comment above), so it is built
// from state.revisionRound the same way, not added as a second static string.
function describeHitlReason(triggers, state = {}) {
  return triggers.map((trigger) => {
    ...
    if (trigger === "critic_flag") {
      const round = state.revisionRound || 0;
      return `critic still requested revision after ${round} round(s)`;
    }
...
// ── Anti-laundering: escalate-only reducer for retrievedSafety (Task N4,
// Item 2) ──
// Verified hazard (found in N3's review): "expand_context" re-scans a FRESH
// `expandedChunks` set on every execution, including every bounded QACritic
// revise round (see the "retrieve"/"expand_context" nodes above -- a revise
// round genuinely re-retrieves with the critic's own additional_queries
// mixed in). Under a plain replace reducer, the retrievedSafety CHANNEL only
// ever reflects the MOST RECENT scan: a round-1 scan that found
// instruction-like or credential-like content in the retrieved evidence
// (`needs_review` -- malicious content WAS fed to the ImpactAnalyst as
// context; that already happened and cannot un-happen) could be silently
// overwritten by a cleaner round-2 scan before decideNextRoute's HITL gate
// (cursor 8, strictly after the revise loop has already resolved) ever reads
// it -- laundering an already-flagged run into a clean-looking one purely by
// chance of which chunks the later round's queries happen to retrieve.
//
// mergeRetrievedSafety fixes this by making the channel escalate-only:
// - status: once either write says "needs_review", the merged status can
//   never read "passed" again for the rest of this run (OR, not replace).
// - risk_types: a SET UNION across writes, not a replace -- a later round
//   that finds a NEW risk type does not erase a type an earlier round found,
//   and a cleaner later round does not shrink the set back down.
// - checks/flagged_files/flagged_sensitive_files/risk_details/detail: merged
//   the same way (per check name: passed only if BOTH rounds passed it,
//   keeping the FLAGGED round's own detail text over a later "all clear"
//   rescan's; flagged file lists are unioned) so every field on the channel
//   stays internally consistent with status/risk_types -- a consumer reading
//   `.checks` or `.detail` never sees a "no risk found" narrative next to a
//   `status: "needs_review"` value. This keeps the channel's SHAPE unchanged
//   (still `{ status, risk_types, risk_details, checks, flagged_files,
//   flagged_sensitive_files, detail }`, the exact shape scanRetrievedSafety()
//   in lib/safety.js already returns), so buildFinalPayload(), guardrails,
//   mergeSafetyReports(), and describeHitlReason() all keep working
//   unmodified -- no consumer needs to change.
//
// On the very FIRST write of a run, `left` is still the Annotation's own
// default (`{ status: "passed", risk_types: [], checks: [] }`, empty
// everywhere) -- merging against that is a no-op that reproduces `right`
// unchanged (empty `left.checks`/`risk_types`/`flagged_files` contribute
// nothing to the union), so a single-round run (the overwhelming majority of
// runs -- the QACritic revise loop is the exception, not the rule) behaves
// byte-for-byte exactly as the old replace reducer did. Only a SECOND write
// within the same run (i.e. only a revise round) can ever observe
// escalate-only behavior diverging from replace.
//
// riskLevel (the ImpactAnalyst's own risk judgment, a separate Annotation
// channel a few lines below) is DELIBERATELY left as a plain replace
// reducer, not converted to this same escalate-only pattern -- see its own
// inline comment below for why; the short version: riskLevel is the
// analyst's evolving JUDGMENT about risk (a legitimate downgrade as better
// evidence arrives each round), while retrievedSafety records an EVENT
// (malicious/sensitive content entered the context this run) that, once it
// has happened, cannot be judged away by a later, cleaner scan. See
// docs/AGENT_RUNTIME_ARCHITECTURE.md for the same distinction spelled out
// for operators reading the finished payload, not just this code.
//
// inputSafety is NOT given this same treatment: it is scanned exactly once,
// at phase 0 ("input_safety", from the user's own question text), and is
// never re-run anywhere in the revise loop or anywhere else in the graph --
// its channel only ever receives ONE write per run, so a replace reducer and
// an escalate-only reducer are observably identical for it and converting it
// would be a no-op change made only for its own sake. Confirmed by reading
// every `.addNode(...)` in this file: "input_safety" is the only node that
// returns an `inputSafety` key.
function mergeRetrievedSafety(left, right) {
  if (!left) return right;
  if (!right) return left;
  const riskTypes = [...new Set([...(left.risk_types || []), ...(right.risk_types || [])])];
  const status = (left.status === "needs_review" || right.status === "needs_review") ? "needs_review" : "passed";

  // Per-check-name merge: AND the two rounds' own `passed` booleans (escalate
  // only), and prefer whichever round's `detail` text explains the FAILURE --
  // if only one round failed a given check, its detail (which names the
  // actual flagged file(s)) survives; if both rounds failed it (rare -- e.g.
  // two different files flagged in two different rounds), both detail
  // strings are kept so neither round's evidence is silently dropped.
  const checksByName = new Map();
  for (const check of [...(left.checks || []), ...(right.checks || [])]) {
    const existing = checksByName.get(check.name);
    if (!existing) {
      checksByName.set(check.name, { ...check });
      continue;
    }
    const merged = { ...existing, passed: existing.passed && check.passed };
    if (existing.passed && check.passed) {
      merged.detail = check.detail;
    } else if (!existing.passed && !check.passed) {
      merged.detail = existing.detail === check.detail ? existing.detail : `${existing.detail} ${check.detail}`;
    } else {
      merged.detail = existing.passed ? check.detail : existing.detail;
    }
    checksByName.set(check.name, merged);
  }
  ...
  return {
    ...
    flagged_files: [...new Set([...(left.flagged_files || []), ...(right.flagged_files || [])])],
    flagged_sensitive_files: [...new Set([...(left.flagged_sensitive_files || []), ...(right.flagged_sensitive_files || [])])],
    detail: riskTypes.length
...
function createGraphStateAnnotation() {
  ...
  return Annotation.Root({
    ...
    // Escalate-only (Task N4, Item 2): see mergeRetrievedSafety()'s own
    // comment block directly above for the full anti-laundering rationale.
    retrievedSafety: Annotation({ reducer: mergeRetrievedSafety, default: () => ({ status: "passed", risk_types: [], checks: [] }) }),
    ...
    // Deliberately plain replace, NOT the escalate-only pattern
    // mergeRetrievedSafety() uses for retrievedSafety above (Task N4, Item 2):
    // riskLevel is the ImpactAnalyst's own evolving JUDGMENT of risk, and a
    // revise round exists precisely so the analyst can RE-ASSESS with better
    // evidence -- a later round downgrading "high" to "medium" once it finds
    // the citation that was missing on round 1 is the loop doing its job, not
    // laundering. retrievedSafety instead records an EVENT that already
    // happened (malicious/sensitive content entered the retrieved context
    // this run) -- that cannot be un-happened by a later, cleaner scan, which
    // is exactly why IT needs escalate-only and this does not.
