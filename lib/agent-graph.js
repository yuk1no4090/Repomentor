...
// linear path at "guardrails". Used both by the "retrieve" node (to decide
// whether it is being re-entered for a revise round) and by decideNextRoute's
// phase-cursor offset below.
...
function decideNextRoute(state) {
  // Phase-cursor-vs-cycle problem: `phase` used to be exactly `state.trace.length`,
  // which only stays aligned with ROUTE_RULES.phaseMap while the graph walks a
  // strictly monotonic 9-step path. The QACritic revise loop (Part B) breaks that
  // assumption -- one revise round re-runs 4 already-counted phase-map nodes
  // (retrieve..qa_plan), so trace.length overshoots phaseMap's indices for every
  // node from the second "retrieve" onward. Rather than add a second, separate
  // cursor channel (more state to keep in sync), `phase` subtracts a
  // trace-derived offset: REVISION_ROUND_NODE_COUNT per revise round already
  // taken (state.revisionRound, bumped by "retrieve" itself the moment it
  // re-enters for a revise round -- see that node below). This keeps `phase`
  // meaningful as "how far along the ORIGINAL 9-phase walk this execution is"
  // even after a loop, with a one-line change instead of a parallel channel that
  // every node would otherwise need to maintain.
  const phase = state.trace.length - (state.revisionRound || 0) * REVISION_ROUND_NODE_COUNT;
  ...
  if (AGENT_GRAPH_MODE === "linear") {
    ...
    graph = graph
      ...
      .addNode("human_review", async (state) => {
        ...
        const reviewRequest = {
          reason: "high risk change requires human review",
        ...
        return {
          hitlRequest: { node: "human_review", reason: "resumed with decision", checkpoint_id: null, decision },
          trace: [makeTraceStep({
    ...
    if (isInterrupted(state)) {
      const pausedTrace = [
        ...
        makeTraceStep({
          ...
          tool: "agent_harness.fallback",
          purpose: "High-risk change flagged for human-in-the-loop review. Execution is genuinely paused at human_review (LangGraph interrupt()) until a reviewer approves or rejects.",
          input: { risk_level: state.riskLevel },
