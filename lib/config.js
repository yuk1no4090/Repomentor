...
// AGENT_MAX_REVISION_ROUNDS bounds the QACritic revise loop (lib/agent-graph.js's
// decideNextRoute): the number of EXTRA retrieve->expand_context->impact_analysis
// ->qa_plan rounds allowed after the first pass, when qaReview.verdict === "revise".
// This must legitimately accept 0 (disable the loop entirely, e.g. for an A/B
// measurement against the default), so it cannot reuse parsePositiveIntegerEnv's
// strictly-positive (value > 0) semantics -- an env value of "0" would silently
// fall back to the default under that helper instead of actually disabling the
// loop. parseNonNegativeIntegerEnv (below) is a sibling helper used only for this
// constant; parsePositiveIntegerEnv itself is unchanged and still used for
// AGENT_MAX_STEPS and every other existing caller.
export const AGENT_MAX_REVISION_ROUNDS = parseNonNegativeIntegerEnv("AGENT_MAX_REVISION_ROUNDS", 1);
export const AGENT_BUDGETS = {
  max_steps: AGENT_MAX_STEPS,
  max_revision_rounds: AGENT_MAX_REVISION_ROUNDS,
  timeout_ms: 30_000,
...
// See AGENT_MAX_REVISION_ROUNDS above for why this sibling of
// parsePositiveIntegerEnv exists: it accepts 0 as a valid explicit override
// (parsePositiveIntegerEnv's `value > 0` guard would silently discard "0" and
// fall back to `fallback` instead), while still rejecting negative or
// non-integer input.
...
// ── Store collection retention caps ──
// store.json's questions/answers/harnessRuns/memoryEvents collections only ever
// grow (unlike authEvents, which normalizeStore() already caps at the last 200
// entries) — a long-running deployment would otherwise accumulate them forever.
// These caps let normalizeStore() slice each collection down to its most recent
// N entries (by insertion order, the same pattern already used for authEvents),
// while staying far above anything the test suite itself produces in a single
// run (peak observed: 22 questions/answers/harnessRuns, 7 memoryEvents).
export const STORE_MAX_QUESTIONS = parsePositiveIntegerEnv("STORE_MAX_QUESTIONS", 500);
export const STORE_MAX_ANSWERS = parsePositiveIntegerEnv("STORE_MAX_ANSWERS", 500);
export const STORE_MAX_HARNESS_RUNS = parsePositiveIntegerEnv("STORE_MAX_HARNESS_RUNS", 200);
export const STORE_MAX_MEMORY_EVENTS = parsePositiveIntegerEnv("STORE_MAX_MEMORY_EVENTS", 200);

// LangGraph checkpoint retention: persistLangGraphCheckpoints() writes a full
// base64 snapshot per run to SQLite (langgraph_checkpoints + the one-row-per-run
// langgraph_checkpoint_payloads table) and never cleaned them up, so a
// long-running deployment's memory.sqlite would grow without bound. This caps
// how many distinct runs' checkpoint history is retained (peak observed in the
// test suite: 15 runs in a single script).
export const CHECKPOINT_MAX_RUNS = parsePositiveIntegerEnv("CHECKPOINT_MAX_RUNS", 50);
...
export const AGENT_TOOL_REGISTRY = [
  ...
  { name: "supervisor_agent.plan_workflow", capability: "workflow_planning", access: "read-only", external_network: false, agent_role: "Supervisor" },
  ...
  { name: "qa_critic_agent.review_impact_analysis", capability: "independent_qa_review", access: "read-only", external_network: false, agent_role: "QACritic" },
