...
| `MEMORY_VECTOR_INDEX_PROVIDER` | unset | Set to `http` for a generic HTTP vector index or `qdrant` for a Qdrant-compatible collection. |
| `MEMORY_VECTOR_INDEX_URL` | unset | Base URL for the external vector index. Generic HTTP calls `/upsert` and `/query`; Qdrant calls `/collections/{collection}/points`. |
| `MEMORY_VECTOR_INDEX_API_KEY` | unset | Optional bearer token for generic HTTP indexes, or Qdrant `api-key` header value. |
| `MEMORY_VECTOR_INDEX_NAMESPACE` | `ai-pm-memory` / `ai_pm_memory` | Namespace or Qdrant collection name passed to the external vector index. |
